...
import { LayoutPanelLeft, Columns2, Copy, Check, Trash2, Minimize2, AlertCircle, CheckCircle2, Wrench, Eye, Upload, Undo2, Redo2 } from 'lucide-react';
...
function Panel({ label, value, onChange: onChangeProp, parsedData, otherParsedData, theme, validationErrors = [], onValidationErrorsChange }) {
  ...
  const handleFormat = useCallback(() => {
    ...
    if (!e) { onChange(f); setScanning(true); setTimeout(() => setScanning(false), 900); }
  }, [value, onChange]);
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b flex-shrink-0 gap-2"
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-[10px] font-mono uppercase tracking-[0.18em]"
            style={{ color: isLight ? '#64748b' : '#64748b' }}>{label}</span>
          {data !== null && !error && <CheckCircle2 size={11} className="text-emerald-400" />}
          {error && value && <AlertCircle size={11} className="text-red-400" />}
        </div>
        ...
        {view === 'raw' && (
          <div className="flex items-center gap-1">
            <button onClick={handleFormat} disabled={!value}
              className={`px-2.5 h-6 text-[10px] font-mono rounded transition-all disabled:opacity-30 ${
                isLight 
            ...
            <button onClick={handleMinify} disabled={!value}
              className={`px-2 h-6 text-[10px] font-mono rounded transition-all disabled:opacity-30 flex items-center gap-1 ${
                isLight 
                  ? 'bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200' 
                  : 'bg-black/[0.05] hover:bg-black/[0.08] text-slate-400 border border-black/[0.07]'
              }`}>
              <Minimize2 size={9} /> Min
            </button>
            ...
            <button onClick={() => fileInputRef.current?.click()}
              className={`px-2 h-6 text-[10px] font-mono rounded transition-all flex items-center gap-1 ${
                isLight 
                  ? 'bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200' 
                  : 'bg-black/[0.05] hover:bg-black/[0.08] text-slate-400 border border-black/[0.07]'
              }`}>
          ...
          <>
              ...
              <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
                <div className="flex flex-col items-center gap-2">
                  <svg width="180" height="90">
                    <rect x="2" y="2" width="176" height="86" rx="8" fill="none"
                      stroke="rgba(59,130,246,0.4)" strokeWidth="1.5" strokeDasharray="8 4"
            ...
            <div className="flex h-full overflow-hidden" style={{ paddingBottom: error ? '100px' : (value ? '28px' : 0) }}>
              ...
              <div className="relative flex-1 overflow-hidden">
                <textarea
                  ...
                  spellCheck={false}
                  autoComplete="off"
                  className="code-editor absolute inset-0 w-full h-full p-3 resize-none overflow-auto font-mono text-[13px] leading-[20px]"
                  style={{
            ...
            {value && !error && (
              <div className="absolute bottom-0 left-0 right-0 flex items-center gap-2 px-4 h-7 border-t"
                ...
                <span className="text-[10px] font-mono" style={{ color: statColor }}>
                  Line: {cursorPos.line}, Column: {cursorPos.column}
                </span>
                <span style={{ color: dotColor }}>·</span>
                {stats ? (
                  <>
                    <span className="text-[10px] font-mono" style={{ color: statColor }}>{stats.keys} keys</span>
                    <span style={{ color: dotColor }}>·</span>
                    <span className="text-[10px] font-mono" style={{ color: statColor }}>depth {stats.depth}</span>
                    <span style={{ color: dotColor }}>·</span>
                    <span className="text-[10px] font-mono" style={{ color: statColor }}>
                  ...
                  <>
                    <span className="text-[10px] font-mono" style={{ color: statColor }}>{value.length} chars</span>
                  </>
                )}
                {data !== null && <span className="text-[10px] font-mono text-emerald-500 ml-auto">✓ Valid</span>}
              </div>
            )}
            ...
            {error && value && (
              <div className="absolute bottom-0 left-0 right-0 flex flex-col border-t"
                ...
                <div className="flex items-center gap-3 px-4 py-2">
                  ...
                  <span className="text-[12px] font-mono text-white flex-1">{error}</span>
                </div>
                <div className="flex items-center gap-3 px-4 pb-3">
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-1.5 bg-white/20 hover:bg-white/30 disabled:opacity-50 disabled:cursor-not-allowed rounded text-[11px] font-mono text-white transition-colors"
                  >
                    <Wrench size={14} />
                    Auto repair
                  </button>
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-1.5 bg-white/10 hover:bg-white/20 rounded text-[11px] font-mono text-white transition-colors"
                  >
                    <Eye size={14} />
                    Show me
                  </button>
...
export default function JsonFormatter() {
  const [leftValue, setLeftValue] = useState(SAMPLE_LEFT);
  const [rightValue, setRightValue] = useState(SAMPLE_RIGHT);
  ...
  useEffect(() => {
    const { data } = parseJSONSafe(rightValue);
    setRightData(data);
  }, [rightValue]);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden mono-grid"
      ...
      {/* Top Ad Banner */}
      <div className="flex-shrink-0 flex justify-center px-4 py-1" style={{ background: bg0 }}>
        <GoogleAd
      ...
      {/* Toolbar */}
      <div className="flex items-center justify-between px-6 h-10 border-b flex-shrink-0"
        style={{ background: bgBar, borderColor }}>
        <div className="flex items-center gap-1">
          {['single', 'split', 'diff'].map(l => (
            <button key={l} onClick={() => setLayout(l)}
              className={`flex items-center gap-1.5 px-3 h-6 rounded text-[11px] font-mono transition-all ${
                layout === l 
              ...
              {l === 'single' && <LayoutPanelLeft size={11} />}
              {l === 'split' && <Columns2 size={11} />}
              {l === 'diff' && <GitCompare size={11} />}
              {l === 'single' ? 'Single' : l === 'split' ? 'Compare' : 'Diff'}
            </button>
          ))}
        </div>
        <span className="text-[10px] font-mono" style={{ color: isLight ? '#94a3b8' : '#374151' }}>
      ...
      {/* Status bar */}
      <div className="flex items-center justify-between px-6 h-7 border-t flex-shrink-0"
        style={{ background: bg0, borderColor }}>
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono" style={{ color: isLight ? '#94a3b8' : '#374151' }}>JSON Forge</span>
          <span style={{ color: isLight ? '#cbd5e1' : '#1f2937' }}>·</span>
          <span className="text-[10px] font-mono" style={{ color: isLight ? '#94a3b8' : '#374151' }}>Optical Workbench v1.0</span>
        </div>
